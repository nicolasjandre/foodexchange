...
export function exchange() {
    function handleClickFirstInputListSuggestions(innerText) {
        Utilities.deleteFirstInputListSuggestion()
        Utilities.deleteSecondInputListSuggestion()
    
        const foodToChangeValue = Matches.findMatches(innerText, foods)
    
        declared.foodToChange = {
        ...
        declared.foodToChangeInput.value = `${declared.foodToChange.description}`
        
    }
    
    function handleClickSecondInputListSuggestions(innerText) {
        Utilities.deleteFirstInputListSuggestion()
        Utilities.deleteSecondInputListSuggestion()
        
        const foodChoosenValue = Matches.findMatches(innerText, foods)
    
        declared.foodChoosen = {
        ...
        declared.foodChoosenInput.value = `${declared.foodChoosen.description}`
    }
    
    function handleClickExchangeButton(event) {
        event = event || window.event
        if (event.type !== 'click') event.preventDefault()
    
        if (declared.foodToChangeInput.value === "" || declared.foodChoosenInput.value === "" || declared.gramsInput.value === "" || declared.gramsInput.value === 0) {
            Utilities.emptyInputErrorModal()
            return
        }
        if (declared.focusProtDiv.classList[1] === undefined && declared.focusCarbDiv.classList[1] === undefined && declared.focusKcalDiv.classList[1] === undefined) {
            Utilities.noPriorityChosenErrorModal()
            return
        }
    ...
    function foodExchange() {
        const foodToChangeGrams = Number(declared.gramsInput.value)
    
        const foodToChangeRecalc = {
            energy_kcal: (foodToChangeGrams * declared.foodToChange.energy_kcal / 100).toFixed(2),
            carbohydrate_g: (foodToChangeGrams * declared.foodToChange.carbohydrate_g / 100).toFixed(1),
            protein_g: (foodToChangeGrams * declared.foodToChange.protein_g / 100).toFixed(1),
            lipid_g: (foodToChangeGrams * declared.foodToChange.lipid_g / 100).toFixed(1)
        }
    
        if (declared.focusProtDiv.classList[1] === 'active') {
            const necessaryProtein = (foodToChangeGrams * declared.foodToChange.protein_g / 100).toFixed()
            const gramsNeedToAchieveThisProtein = (necessaryProtein / declared.foodChoosen.protein_g * 100).toFixed()
    
            const foodChoosenRecalc = {
                energy_kcal: (gramsNeedToAchieveThisProtein * declared.foodChoosen.energy_kcal / 100).toFixed(2),
                carbohydrate_g: (gramsNeedToAchieveThisProtein * declared.foodChoosen.carbohydrate_g / 100).toFixed(1),
                protein_g: (gramsNeedToAchieveThisProtein * declared.foodChoosen.protein_g / 100).toFixed(1),
                lipid_g: (gramsNeedToAchieveThisProtein * declared.foodChoosen.lipid_g / 100).toFixed(1)
            }
    
            displayExchangeResult(foodToChangeRecalc, foodChoosenRecalc, foodToChangeGrams, gramsNeedToAchieveThisProtein)
        }
        if (declared.focusCarbDiv.classList[1] === 'active') {
            const necessaryCarb = (foodToChangeGrams * declared.foodToChange.carbohydrate_g / 100).toFixed()
            const gramsNeedToAchieveThisCarb = (necessaryCarb / declared.foodChoosen.carbohydrate_g * 100).toFixed()
    
            const foodChoosenRecalc = {
                energy_kcal: (gramsNeedToAchieveThisCarb * declared.foodChoosen.energy_kcal / 100).toFixed(2),
                carbohydrate_g: (gramsNeedToAchieveThisCarb * declared.foodChoosen.carbohydrate_g / 100).toFixed(1),
                protein_g: (gramsNeedToAchieveThisCarb * declared.foodChoosen.protein_g / 100).toFixed(1),
                lipid_g: (gramsNeedToAchieveThisCarb * declared.foodChoosen.lipid_g / 100).toFixed(1)
            }
    
            displayExchangeResult(foodToChangeRecalc, foodChoosenRecalc, foodToChangeGrams, gramsNeedToAchieveThisCarb)
        }
        if (declared.focusKcalDiv.classList[1] === 'active') {
            const necessaryKcal = (foodToChangeGrams * declared.foodToChange.energy_kcal / 100).toFixed()
            const gramsNeedToAchieveThisKcal = (necessaryKcal / declared.foodChoosen.energy_kcal * 100).toFixed()
    
            const foodChoosenRecalc = {
                energy_kcal: (gramsNeedToAchieveThisKcal * declared.foodChoosen.energy_kcal / 100).toFixed(2),
                carbohydrate_g: (gramsNeedToAchieveThisKcal * declared.foodChoosen.carbohydrate_g / 100).toFixed(1),
                protein_g: (gramsNeedToAchieveThisKcal * declared.foodChoosen.protein_g / 100).toFixed(1),
                lipid_g: (gramsNeedToAchieveThisKcal * declared.foodChoosen.lipid_g / 100).toFixed(1)
            }
    
            displayExchangeResult(foodToChangeRecalc, foodChoosenRecalc, foodToChangeGrams, gramsNeedToAchieveThisKcal)
        }
    }
    function displayExchangeResult(food1, food2, food1Grams, food2Grams) {
        ...
        declared.sectionResults.innerHTML = `
            <div class="tables">
                ...
                <table>
                        <caption>${food1Grams}g de ${declared.foodToChange.description}</caption>
                        <colgroup>
                    ...
                    <table>
                        <caption>${food2Grams}g de ${declared.foodChoosen.description}</caption>
                        <colgroup>
                            <col class="firstColumn">
                            <col class="secondColumn">
                            <col class="thirdColumn">
        ...
        closeBtn.addEventListener('click', Utilities.closeResultModal)
        closeBtn.addEventListener('touchstart', Utilities.closeResultModal)
    }
    
    function handleClickFocusCarb(event) {
        event = event || window.event
        if (event.type !== 'click') event.preventDefault()
    
        declared.focusCarbDiv.classList.toggle('active')
        declared.focusProtDiv.classList.remove('active')
        declared.focusKcalDiv.classList.remove('active')
    }
    
    function handleClickFocusProt(event) {
        event = event || window.event
        if (event.type !== 'click') event.preventDefault()
    
        declared.focusProtDiv.classList.toggle('active')
        declared.focusCarbDiv.classList.remove('active')
        declared.focusKcalDiv.classList.remove('active')
    }
    
    function handleClickFocusKcal(event) {
        event = event || window.event
        if (event.type !== 'click') event.preventDefault()
    
        declared.focusKcalDiv.classList.toggle('active')
        declared.focusProtDiv.classList.remove('active')
        declared.focusCarbDiv.classList.remove('active')
    }
    
    return {
        handleClickFirstInputListSuggestions,
        handleClickSecondInputListSuggestions,
        handleClickExchangeButton,
        handleClickFocusCarb,
        handleClickFocusKcal,
        handleClickFocusProt
    }
}